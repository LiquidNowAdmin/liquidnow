-- Add Creditreform number (Crefo) to companies table
-- Required by Qred (DE market) as uniqueCompanyIdentifier
alter table companies add column if not exists crefo text;

-- Update get_or_create_company to accept crefo parameter
...
begin
  ...
  if v_company_id is null then
    insert into companies (tenant_id, name, hrb, ust_id, crefo, website, address, annual_revenue)
    values (
      v_tenant_id,
      p_name,
      nullif(p_hrb, ''),
