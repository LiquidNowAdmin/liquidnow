-- ============================================
-- Fix: log_event() trigger for tables without status column
-- (providers, products have no status column)
-- ============================================
...
begin
  ...
  if TG_OP = 'INSERT' then
    ...
    event_row := new;
    ev_old_json := to_jsonb(old);
    -- Safe status check via JSONB (status column may not exist on all tables)
    if ev_old_json ? 'status'
       and (ev_old_json->>'status') is distinct from (to_jsonb(new)->>'status') then
  ...
  if TG_OP = 'UPDATE' then
    ev_payload := jsonb_build_object(
      'old', coalesce(ev_old_json, to_jsonb(old)),
      'new', to_jsonb(new)
    );
  else
